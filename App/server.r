...
server <- function(input, output, session) {
 ...
 # Reactive function untuk mengambil data novel
  ...
  validate(
    need(nrow(filtered_novel_data) > 0, "Data tidak tersedia")
  )
  ...
  # Batasi panjang deskripsi dengan "Read More" -> "Show Less"
  filtered_novel_data$Deskripsi <- sapply(1:nrow(filtered_novel_data), function(i) {
    deskripsi <- filtered_novel_data$deskripsi[i]
    if (nchar(deskripsi) > 100) {
      short_text <- substr(deskripsi, 1, 100)
      full_text <- deskripsi
      return(paste0(
        '<span id="short_', i, '">', short_text, '...</span> ',
        '<span id="full_', i, '" style="display: none;">', full_text, '</span> ',
        '<a href="#" id="toggle_', i, '" onclick="toggleText(', i, '); return false;">Read More</a>'
      ))
    } else {
      return(deskripsi)
    }
...
output$reviewTable <- renderDataTable({
  ...
  review_table <- filtered_review_data()
  
  # Batasi panjang ulasan dengan "Read More" -> "Show Less"
  review_table$Ulasan <- sapply(1:nrow(review_table), function(i) {
    ulasan <- review_table$ulasan[i]
    if (nchar(ulasan) > 100) {
      short_text <- substr(ulasan, 1, 100)
      full_text <- ulasan
      return(paste0(
        '<span id="short_', i, '">', short_text, '...</span> ',
        '<span id="full_', i, '" style="display: none;">', full_text, '</span> ',
  ...
  # Pilih kolom yang akan ditampilkan
  review_table <- review_table %>% select(
    "Judul Novel" = judul, 
  ...
  datatable(review_table, 
            options = list(pageLength = 10, autoWidth = TRUE), 
            escape = FALSE,  # Aktifkan render HTML untuk menampilkan tombol Read More
            class = "display")
})
